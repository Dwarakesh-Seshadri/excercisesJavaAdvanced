...
public class MemoryPersonRepository implements PersonRepository {
    ...
    public MemoryPersonRepository()  {
        infoLogger.info("List is fully created");
        Person.counter=0;
        ...
        Person p4 = new Person("Willem-Alexander","Janssen",LocalDate.of(1971, 1, 18),"willemalexander.janssen@klm.nl","somepass8","nl",c4);
    ...
    public Person findPersonById(int id) throws PersonNotFoundException {
        Person p1 = persons.stream()
                .filter(person1 -> person1.getPersonNumber()==id)
                .findFirst()
                .orElseThrow(() -> new PersonNotFoundException("Person with the mentioned id is not found"));
        return p1;
    }
    ...
    public Person findPersonByEmailAndPassword(String email, String password) throws PersonNotFoundException
    {
        Person p1 = persons.stream()
                .filter(person1 ->
                        person1.getEmail().equalsIgnoreCase(email) && person1.getPassword().equalsIgnoreCase(password))
                .findFirst()
                .orElseThrow(() -> new PersonNotFoundException("Person with the mentioned email and psw is not found"));
    ...
    public List<Person> findPersonsForCompany(String companyName) throws PersonNotFoundException {
        ...
        List<Person> personList =  persons.stream()
                                            .filter(person1 -> person1.getCompany()!=null)
                                            .filter(person1 ->
                                                    person1.getCompany().getName().equalsIgnoreCase(companyName))
                                            .collect(Collectors.toList());

        if (personList.isEmpty()){
            throw new PersonNotFoundException("No persons found working for the :" + companyName);
        }
        return personList;
    }

    @Override
    public void addPerson(Person p) throws PersonAlreadyExistException {

        if(!persons.contains(p)){
            persons.add(p);
        }else  {
            throw new PersonAlreadyExistException("This person already exists in the list");
        }
